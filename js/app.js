...
function checkButtonClassType(btn, className) {
  console.log(btn, " <-  this is button");
...
function renderCalculatorDisplay() {
  ...
  displayEl.innerText = `${firstNumber} ${selectedOperation} ${secondNumber}`;
}

function renderAnswer() {
  displayEl.innerText = calculate[selectedOperation](firstNumber, secondNumber);
}


function handleKeyPress(e) {
	console.log(e.key, " <- this is key!");
  
	if ("*+-/".includes(e.key)) {
	  console.log("User is pressing an operand!");
	  selectedOperation = e.key;
	  console.log(selectedOperation);
	  renderCalculatorDisplay();
	} else if ("=Enter".includes(e.key)) {
	  console.log("User is pressing =");
	  renderAnswer();
	} else if ("1234567890".includes(e.key)) {
	  console.log("user is pressing a number");
	  if (firstNumber === "") {
		firstNumber = parseInt(e.key);
	  } else {
		secondNumber = parseInt(e.key);
	  }
  
	  // displayEl.innerText += firstNumber;
	  renderCalculatorDisplay();
	} else if ("c".includes(e.key)) {
	  console.log("user is pressing c");
  ...
  function handleClick(e) {
	...
	if (e.target.classList.contains("operand")) {
	  ...
	  //
	} else if (checkButtonClassType(e.target, "number")) {
	  if (firstNumber === "") {
		firstNumber = parseInt(e.target.innerText);
	  } else {
		secondNumber = parseInt(e.target.innerText);
	  }
  
	  // displayEl.innerText += firstNumber;
	  renderCalculatorDisplay();
	} else if (checkButtonClassType(e.target, "clear")) {
...
function add(...args){
	// change args into an array in order to use the array's reduce method	
	const arrayOfNums = Array.from(args)
